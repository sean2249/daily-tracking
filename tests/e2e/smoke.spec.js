...
// vertically (only the inner .scroll list scrolls). Runs offline against the
// Auth shell — the same .app-shell column every screen lives in.
...
for (const vp of mobileViewports) {
  test(`no horizontal/vertical document overflow at ${vp.width}x${vp.height}`, async ({ page }) => {
    ...
    // The column must FILL the viewport width on phones (max-width 440 > 390/360).
    expect(metrics.shellWidth, 'app shell narrower than viewport').toBeGreaterThanOrEqual(metrics.innerWidth - 1);
  });
}
